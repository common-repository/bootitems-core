...
if ( bc_fs()->can_use_premium_code() ) {
	class Bootitems_Core_About_Info_Widget extends WP_Widget {
		
		function __construct() {
			parent::__construct(
			
			// Base ID of your widget
			'bootitems_core_about_info',
			
			// Widget name will appear in UI
			esc_html__('Bootitems - About Info', 'bootitems-core'),
			
			// Widget description
			array( 'description' => esc_html__( 'A widget that displays logo with short description.', 'bootitems-core' ), )
			);
		}
		...
		public function form( $instance ) {
			...
			<!-- image url -->
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>"><?php _e( 'Image URL:', 'bootitems-core' ) ?></label>
			...
			<!-- description -->
			<p>
				<label for="<?php echo esc_attr($this->get_field_id( 'description' )); ?>"><?php _e( 'Description:', 'bootitems-core' ); ?></label>
	...
	function bootitems_core_load_about_widget() {
		register_widget( 'Bootitems_Core_About_Info_Widget' );
	}
	add_action( 'widgets_init', 'bootitems_core_load_about_widget' );
}